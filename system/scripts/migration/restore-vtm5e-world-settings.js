export const RestoreOldWorldSettings = async function () {
  ...
  if (worldSettings.length > 0 && !game.settings.get('aztharion', 'settingsMigrationComplete')) {
    // Begin migration
    ...
    worldSettings.forEach((setting) => {
      // Snip out the 'vtm5e' part to get the right 'aztharion' key, and then set the value
      // to complete the migration
      game.settings.set('aztharion', setting.key.replace('vtm5e.', ''), setting.value)
    })

    // Complete migration
    ui.notifications.info('World settings migration complete.')
    game.settings.set('aztharion', 'settingsMigrationComplete', true)
  }
}
