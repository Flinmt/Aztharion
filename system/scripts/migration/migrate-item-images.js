export const MigrateItemImages = async function () {
  ...
  for (const actor of actorsList) {
    ...
    for (const item of actorItems) {
      // Check if there are any instances of /systems/aztharion/assets/icons/powers/ in the actor data
      if (countInstances(item.img, 'systems/aztharion/assets/icons/powers/') > 0) {
        hasFixedItems = true
        ...
        const updatedItem = {
          ...
          img: item.img.replace(
            /\/systems\/aztharion\/assets\/icons\/powers\//,
            'systems/aztharion/assets/icons/items/'
          )
        }
  ...
  function countInstances(string) {
    ...
    // Regex
    const regex = /\/systems\/aztharion\/assets\/icons\/powers\//g
