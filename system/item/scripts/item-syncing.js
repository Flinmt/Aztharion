export const _onSyncFromDataItem = async function (event) {
  ...
  const dataItemId = item.getFlag('aztharion', 'dataItemId')
  ...
  for (const compendium of compendiumsList) {
    ...
    const foundItems = docs.filter((item) => item?.flags?.aztharion?.dataItemId === dataItemId)

    compendiumDataItems.push(...foundItems)
  }

  const worldDataItems = game.items.filter(
    (item) => item.getFlag('aztharion', 'dataItemId') === dataItemId
  )
...
export const _onSyncToDataItems = async function (event) {
  ...
  if (!dataItemId) {
    return ui.notifications.info(game.i18n.localize('AZTHARION.ItemsList.DataItemIdCannotBeEmpty'))
  }
  ...
  for (const actor of actorsList) {
    const foundItems = actor.items.filter((item) => item?.flags?.aztharion?.dataItemId === dataItemId)
  ...
  if (totalCount === 0) {
    return ui.notifications.info(
      game.i18n.format('AZTHARION.ItemsList.NoItemsFound', {
        dataItemId
      })
    )
  } else {
    ...
    const content = `<p>
      ${game.i18n.format('AZTHARION.ItemsList.ConfirmOverwriteString', {
        string: totalCount
      })}
    </p>`
    ...
    const updateItemsConfirmed = await foundry.applications.api.DialogV2.wait({
      window: {
        title: game.i18n.localize('AZTHARION.ItemsList.ConfirmOverwrite')
      },
